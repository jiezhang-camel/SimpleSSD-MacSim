...
static double SIM_array_dec_select_cap(u_int n_input) {
  ...
  Ctotal = n_input * SIM_draincap(WdecNORn, NCH, 1) +
           SIM_draincap(WdecNORp, PCH, n_input);
...
static double SIM_array_dec_chgaddr_cap(u_int n_gates) {
  ...
  Ctotal += (SIM_draincap(Wdecinvp, PCH, 1) + SIM_draincap(Wdecinvn, NCH, 1) +
             SIM_gatecap(Wdecinvp, 1) + SIM_gatecap(Wdecinvn, 1)) /
            2;
 ...
 */
static double SIM_array_dec_chgl1_cap(u_int n_in_1st, u_int n_in_2nd,
                                      u_int n_gates) {
  double Ctotal;

  /* part 1: drain cap of level-1 decoder */
  Ctotal = n_in_1st * SIM_draincap(Wdec3to8p, PCH, 1) +
           SIM_draincap(Wdec3to8n, NCH, n_in_1st);
...
static int SIM_array_dec_init(SIM_array_dec_t *dec, int model, u_int n_bits) {
  if ((dec->model = model) && model < DEC_MAX_MODEL) {
    ...
    if (n_bits >= 2) {
      dec->e_chg_l1 = SIM_array_dec_chgl1_cap(dec->n_in_1st, dec->n_in_2nd,
                                              dec->n_out_1st) *
                      EnergyFactor;
      if (n_bits >= 4)
        dec->e_chg_output =
            SIM_array_dec_select_cap(dec->n_in_2nd) * EnergyFactor;
    }
 ...
 * Return value: 0
 */
int SIM_array_dec_record(SIM_array_dec_t *dec, LIB_Type_max_uint prev_addr,
                         LIB_Type_max_uint curr_addr) {
...
double SIM_array_dec_report(SIM_array_dec_t *dec) {
  ...
  Etotal = dec->n_chg_output * dec->e_chg_output +
           dec->n_chg_l1 * dec->e_chg_l1 + dec->n_chg_addr * dec->e_chg_addr;
 ...
 */
static double SIM_array_wordline_cap(u_int cols, double wire_cap,
                                     double tx_width) {
  double Ctotal, Cline, psize, nsize;

  /* part 1: line cap, including gate cap of pass tx's and metal cap */
  Ctotal = Cline =
      SIM_gatecappass(tx_width, BitWidth / 2 - tx_width) * cols + wire_cap;
 ...
 */
static int SIM_array_wordline_init(SIM_array_wordline_t *wordline, int model,
                                   int share_rw, u_int cols, double wire_cap,
                                   u_int end) {
  if ((wordline->model = model) && model < WORDLINE_MAX_MODEL) {
    ...
    switch (model) {
      case CAM_RW_WORDLINE:
        wordline->e_read =
            SIM_cam_wordline_cap(cols * end, wire_cap, Wmemcellr) *
            EnergyFactor;
        if (wordline->share_rw = share_rw)
          ...
          wordline->e_write =
              SIM_cam_wordline_cap(cols * 2, wire_cap, Wmemcellw) *
              EnergyFactor;
        break;
      ...
      case CAM_WO_WORDLINE: /* only have write wordlines */
        ...
        wordline->e_read = 0;
        wordline->e_write =
            SIM_cam_wordline_cap(cols * 2, wire_cap, Wmemcellw) * EnergyFactor;
        break;
      ...
      case CACHE_WO_WORDLINE: /* only have write wordlines */
        ...
        wordline->e_read = 0;
        wordline->e_write =
            SIM_array_wordline_cap(cols * 2, wire_cap, Wmemcellw) *
            EnergyFactor;
        break;

      case CACHE_RW_WORDLINE:
        wordline->e_read =
            SIM_array_wordline_cap(cols * end, wire_cap, Wmemcellr) *
            EnergyFactor;
        if (wordline->share_rw = share_rw)
          wordline->e_write = wordline->e_read;
        else
        ...
        /* input driver */
        wordline->I_static =
            (Woutdrivern * NMOS_TAB[0] + Woutdriverp * PMOS_TAB[0]);
        break;
 ...
 */
int SIM_array_wordline_record(SIM_array_wordline_t *wordline, int rw,
                              LIB_Type_max_uint n_switch) {
  if (rw)
    wordline->n_write += n_switch;
  else
    wordline->n_read += n_switch;
 ...
 *   n_colsel_pre  -- # of precharge transistor drains for 1 column selector, if
 * any pre_size      -- width of precharge transistors outdrv_model  -- output
 * driver model type
 *
 * Return value: switching cap
 *
 ...
 */
static double SIM_array_column_read_cap(u_int rows, double wire_cap, u_int end,
                                        u_int n_share_amp, u_int n_bitline_pre,
                                        u_int n_colsel_pre, double pre_size,
                                        int outdrv_model) {
  double Ctotal;
...
static double SIM_array_share_column_write_cap(u_int rows, double wire_cap,
                                               u_int n_share_amp,
                                               u_int n_bitline_pre,
                                               double pre_size) {
  double Ctotal, psize, nsize;
  ...
  if (n_share_amp > 1)
    Ctotal += SIM_draincap(Wbitmuxn, NCH, 1);
  else
    Ctotal += 2 * SIM_gatecap(WsenseQ1to4, 10);
...
/* one bitline switches twice in one cycle, so no 1/2 */
static double SIM_array_share_column_read_cap(u_int rows, double wire_cap,
                                              u_int n_share_amp,
                                              u_int n_bitline_pre,
                                              u_int n_colsel_pre,
                                              double pre_size) {
  ...
  Ctotal = SIM_array_share_column_write_cap(rows, wire_cap, n_share_amp,
                                            n_bitline_pre, pre_size);
...
static int SIM_array_bitline_init(SIM_array_bitline_t *bitline, int model,
                                  int share_rw, u_int end, u_int rows,
                                  double wire_cap, u_int n_share_amp,
                                  u_int n_bitline_pre, u_int n_colsel_pre,
                                  double pre_size, int outdrv_model) {
    ...
    switch (model) {
      case RW_BITLINE:
        ...
        if (bitline->share_rw = share_rw) {
          ...
          bitline->e_col_read = SIM_array_share_column_read_cap(
                                    rows, wire_cap, n_share_amp, n_bitline_pre,
                                    n_colsel_pre, pre_size) *
                                SenseEnergyFactor;
          bitline->e_col_write =
              SIM_array_share_column_write_cap(rows, wire_cap, n_share_amp,
                                               n_bitline_pre, pre_size) *
              EnergyFactor;
        }
        else {
          bitline->e_col_read =
              SIM_array_column_read_cap(rows, wire_cap, end, n_share_amp,
                                        n_bitline_pre, n_colsel_pre, pre_size,
                                        outdrv_model) *
              (end == 2 ? SenseEnergyFactor : EnergyFactor);
          bitline->e_col_write =
              SIM_array_column_write_cap(rows, wire_cap) * EnergyFactor;

          /* static power */
          bitline->I_static =
              2 * (Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]);
        }
        break;
...
int SIM_array_bitline_record(SIM_array_bitline_t *bitline, int rw, u_int cols,
                             LIB_Type_max_uint old_value,
                             LIB_Type_max_uint new_value) {
...
static double SIM_array_comp_match_cap(u_int n_bits) {
  return (n_bits *
          (SIM_draincap(Wcompn, NCH, 1) + SIM_draincap(Wcompn, NCH, 2)));
}
...
static double SIM_array_comp_miss_cap(u_int assoc) {
  ...
  return (assoc * SIM_draincap(WdecNORn, NCH, 1) +
          SIM_draincap(WdecNORp, PCH, assoc));
}
...
static int SIM_array_comp_init(SIM_array_comp_t *comp, int model, u_int n_bits,
                               u_int assoc, u_int n_pre, double matchline_len,
                               double tagline_len) {
  if ((comp->model = model) && model < COMP_MAX_MODEL) {
    ...
    switch (model) {
      ...
      case CAM_COMP:
        ...
        comp->e_chg_addr =
            SIM_cam_comp_tagline_cap(assoc, tagline_len) * EnergyFactor;
        comp->e_mismatch =
            SIM_cam_comp_mismatch_cap(n_bits, n_pre, matchline_len) *
            EnergyFactor;
...
int SIM_array_comp_global_record(SIM_array_comp_t *comp,
                                 LIB_Type_max_uint prev_value,
                                 LIB_Type_max_uint curr_value, int miss) {
  if (miss)
    comp->n_miss++;
  ...
  switch (comp->model) {
    case CACHE_COMP:
      comp->n_access++;
      comp->n_chg_addr +=
          SIM_Hamming(prev_value, curr_value, comp->comp_mask) * comp->assoc;
      break;
...
/* recover means prev_tag will recover on next cycle, e.g. driven by sense
 * amplifier */
/* return value: 1 if miss, 0 if hit */
int SIM_array_comp_local_record(SIM_array_comp_t *comp,
                                LIB_Type_max_uint prev_tag,
                                LIB_Type_max_uint curr_tag,
                                LIB_Type_max_uint input, int recover) {
  u_int H_dist;
  int mismatch;

  if (mismatch = (curr_tag != input))
    comp->n_mismatch++;
...
double SIM_array_comp_report(SIM_array_comp_t *comp) {
  return (comp->n_access * comp->e_access + comp->n_match * comp->e_match +
          ...
          comp->n_bit_match * comp->e_bit_match +
          comp->n_chg_addr * comp->e_chg_addr +
...
static double SIM_array_mux_mismatch_cap(u_int n_nor_gates) {
  ...
  Cmul +=
      n_nor_gates * (SIM_gatecap(WmuxdrvNORn, 1) + SIM_gatecap(WmuxdrvNORp, 1));
  /* drain cap of nor gates, only count 1 */
  Cmul +=
      SIM_draincap(WmuxdrvNORp, PCH, 2) + 2 * SIM_draincap(WmuxdrvNORn, NCH, 1);
...
static int SIM_array_mux_init(SIM_array_mux_t *mux, int model, u_int n_gates,
                              u_int assoc) {
...
int SIM_array_mux_record(SIM_array_mux_t *mux, LIB_Type_max_uint prev_addr,
                         LIB_Type_max_uint curr_addr, int miss) {
...
double SIM_array_mux_report(SIM_array_mux_t *mux) {
  return (mux->n_mismatch * mux->e_mismatch +
          mux->n_chg_addr * mux->e_chg_addr);
}
...
static double SIM_array_outdrv_select_cap(u_int data_width) {
  ...
  Ctotal = SIM_gatecap(Woutdrvseln, 1) + SIM_gatecap(Woutdrvselp, 1) +
           SIM_draincap(Woutdrvseln, NCH, 1) +
           SIM_draincap(Woutdrvselp, PCH, 1);

  /* stage 2: gate cap of nand gate and nor gate */
  /* only consider 1 gate cap because another and drain cap switch depends on
   * data value */
  Ctotal += data_width *
            (SIM_gatecap(Woutdrvnandn, 1) + SIM_gatecap(Woutdrvnandp, 1) +
             SIM_gatecap(Woutdrvnorn, 1) + SIM_gatecap(Woutdrvnorp, 1));
...
static double SIM_array_outdrv_outdata_cap(u_int value) {
  ...
  if (value)
    ...
    Ctotal = SIM_draincap(Woutdrvnandn, NCH, 2) +
             2 * SIM_draincap(Woutdrvnandp, PCH, 1);
  else
    /* drain cap of nor gate */
    Ctotal = 2 * SIM_draincap(Woutdrvnorn, NCH, 1) +
             SIM_draincap(Woutdrvnorp, PCH, 2);
...
static int SIM_array_outdrv_init(SIM_array_out_t *outdrv, int model,
                                 u_int item_width) {
...
int SIM_array_outdrv_global_record(SIM_array_out_t *outdrv,
                                   LIB_Type_max_uint data) {
  u_int n_1;
...
/* recover means prev_data will recover on next cycle, e.g. driven by sense
 * amplifier */
/* NOTE: this function SHOULD not be called by a fully-associative cache */
int SIM_array_outdrv_local_record(SIM_array_out_t *outdrv,
                                  LIB_Type_max_uint prev_data,
                                  LIB_Type_max_uint curr_data, int recover) {
  if (recover)
    outdrv->n_chg_data +=
        2 * SIM_Hamming(prev_data, curr_data, outdrv->out_mask);
  else
...
double SIM_array_outdrv_report(SIM_array_out_t *outdrv) {
  return (outdrv->n_select * outdrv->e_select +
          outdrv->n_chg_data * outdrv->e_chg_data +
          outdrv->n_out_1 * outdrv->e_out_1 +
          outdrv->n_out_0 * outdrv->e_out_0);
}
...
static double SIM_array_mem_cap(u_int read_ports, u_int write_ports,
                                int share_rw, u_int end) {
  double Ctotal;
  ...
  Ctotal += SIM_draincap(Wmemcella, NCH, 1) +
            SIM_draincap(Wmemcella * Wmemcellbscale, PCH, 1);

  /* has coefficient ( 1/2 * 2 ) */
  /* part 3: gate capacitance of memory cell */
  Ctotal +=
      SIM_gatecap(Wmemcella, 1) + SIM_gatecap(Wmemcella * Wmemcellbscale, 1);
...
static int SIM_array_mem_init(SIM_array_mem_t *mem, int model, u_int read_ports,
                              u_int write_ports, int share_rw, u_int end) {
  double I_static;
  ...
  if ((mem->model = model) && model < MEM_MAX_MODEL) {
    ...
    switch (model) {
      case CAM_TAG_RW_MEM:
        mem->e_switch = SIM_cam_tag_mem_cap(read_ports, write_ports, share_rw,
                                            end, SIM_ARRAY_RW) *
                        EnergyFactor;
        break;

        /* FIXME: it's only an approximation using CAM_TAG_WO_MEM to emulate
         * CAM_ATTACH_MEM */
      case CAM_ATTACH_MEM:
      case CAM_TAG_WO_MEM:
        mem->e_switch = SIM_cam_tag_mem_cap(read_ports, write_ports, share_rw,
      ...
      case CAM_DATA_MEM:
        mem->e_switch =
            SIM_cam_data_mem_cap(read_ports, write_ports) * EnergyFactor;
        break;

      default: /* NORMAL_MEM */
        mem->e_switch =
            SIM_array_mem_cap(read_ports, write_ports, share_rw, end) *
            EnergyFactor;

        /* static power */
        I_static = 0;
        /* memory cell */
        I_static += (Wmemcella * NMOS_TAB[0] +
                     Wmemcella * Wmemcellbscale * PMOS_TAB[0]) *
                    2;
        /* read port pass tx */
...
int SIM_array_mem_record(SIM_array_mem_t *mem, LIB_Type_max_uint prev_value,
                         LIB_Type_max_uint curr_value, u_int width) {
...
static int SIM_array_pre_init(SIM_array_pre_t *pre, int model,
                              double pre_size) {
  u_int n_gate;
...
int SIM_array_init(SIM_array_info_t *info, int is_fifo, u_int n_read_port,
                   u_int n_write_port, u_int n_entry, u_int line_width,
                   int outdrv, int arr_buf_type) {
  /* ==================== set parameters ==================== */
  /* general parameters */
  info->share_rw = 0;
  ...
  // if (write_port) SIM_buf_port_state_init(info, write_port,
  // info->write_ports);
  ...
  if (info->arr_buf_type == SRAM) {
    ...
    if (dec_width = SIM_logtwo(info->n_set)) {
      ...
      if (info->data_end == 2)
        ...
        wordline_len = cols * (RegCellWidth +
                               (2 * ports - info->read_ports) * BitlineSpacing);
      ...
      Cline =
          rows * SIM_draincap(Wmemcellr, NCH, 1) + bitline_cmetal * bitline_len;
      ...
      n_colsel_pre = (info->data_n_share_amp > 1)
                         ? SIM_array_n_pre_drain(info->data_colsel_pre_model)
                         : 0;
      SIM_array_bitline_init(&arr->data_bitline, info->data_bitline_model,
                             info->share_rw, info->data_end, rows,
                             bitline_len * bitline_cmetal,
                             info->data_n_share_amp, n_bitline_pre,
                             n_colsel_pre, pre_size, info->outdrv_model);
      /* static power */
      arr->I_static += arr->data_bitline.I_static * cols * info->write_ports;

      /* bitline precharging power initialization */
      SIM_array_pre_init(&arr->data_bitline_pre, info->data_bitline_pre_model,
                         pre_size);
      /* static power */
      ...
      if (info->data_n_share_amp > 1)
        SIM_array_pre_init(&arr->data_colsel_pre, info->data_colsel_pre_model,
                           pre_size);
      ...
      bitline_len =
          rows * (RegCellHeight + info->write_ports * WordlineSpacing);
      wordline_len =
          cols * (RegCellWidth + 2 * info->write_ports * BitlineSpacing);
      info->data_arr_width = wordline_len;
      info->data_arr_height = bitline_len;

      /* bitline power initialization */
      SIM_array_bitline_init(&arr->data_bitline, info->data_bitline_model, 0,
                             info->data_end, rows, bitline_len * bitline_cmetal,
                             1, 0, 0, 0, info->outdrv_model);
    }

    /* wordline power initialization */
    SIM_array_wordline_init(&arr->data_wordline, info->data_wordline_model,
                            info->share_rw, cols,
                            wordline_len * wordline_cmetal, info->data_end);
    /* static power */
    ...
    SIM_array_mem_init(&arr->data_mem, info->data_mem_model, info->read_ports,
                       info->write_ports, info->share_rw, info->data_end);
    /* static power */
    ...
    /* BEGIN: legacy */
    // GLOB(e_const)[CACHE_PRECHARGE] = ndwl * ndbl * SIM_array_precharge_power(
    // PARM( n_pre_gate ), pre_size, 10 ) * cols * ((( n_share_amp > 1 ) ? ( 1.0
    // / n_share_amp ):0 ) + 1 );
    /* END: legacy */
    ...
    if (info->tag_mem_model) {
      if (info->n_set > 1) {
        ...
        if (info->tag_end == 2)
          ...
          wordline_len = cols * (RegCellWidth + (2 * ports - info->read_ports) *
                                                    BitlineSpacing);
        ...
        /* compute precharging size */
        /* FIXME: should consider n_pre and pre_size simultaneously */
        Cline = rows * SIM_draincap(Wmemcellr, NCH, 1) +
        ...
        n_colsel_pre = (info->tag_n_share_amp > 1)
                           ? SIM_array_n_pre_drain(info->tag_colsel_pre_model)
                           : 0;
        SIM_array_bitline_init(&arr->tag_bitline, info->tag_bitline_model,
                               info->share_rw, info->tag_end, rows,
                               bitline_len * bitline_cmetal,
                               info->tag_n_share_amp, n_bitline_pre,
                               n_colsel_pre, pre_size, SIM_NO_MODEL);

        /* bitline precharging power initialization */
        SIM_array_pre_init(&arr->tag_bitline_pre, info->tag_bitline_pre_model,
                           pre_size);
        /* bitline column selector precharging power initialization */
        if (info->tag_n_share_amp > 1)
          SIM_array_pre_init(&arr->tag_colsel_pre, info->tag_colsel_pre_model,
                             pre_size);
        ...
        bitline_len = rows * (CamCellHeight + ports * WordlineSpacing +
                              info->read_ports * MatchlineSpacing);
        if (info->tag_end == 2)
          wordline_len = cols * (CamCellWidth + 2 * ports * BitlineSpacing +
                                 2 * info->read_ports * TaglineSpacing);
        else /* info->tag_end == 1 */
          wordline_len =
              cols *
              (CamCellWidth + (2 * ports - info->read_ports) * BitlineSpacing +
               2 * info->read_ports * TaglineSpacing);
        info->tag_arr_width = wordline_len;
        info->tag_arr_height = bitline_len;
        ...
        if (is_rw_bitline(info->tag_bitline_model)) {
          ...
          SIM_array_bitline_init(&arr->tag_bitline, info->tag_bitline_model,
                                 info->share_rw, info->tag_end, rows,
                                 bitline_len * bitline_cmetal, 1, n_bitline_pre,
                                 0, pre_size, SIM_NO_MODEL);
          ...
          /* bitline power initialization */
          SIM_array_bitline_init(
              &arr->tag_bitline, info->tag_bitline_model, 0, info->tag_end,
              rows, bitline_len * bitline_cmetal, 1, 0, 0, 0, SIM_NO_MODEL);
        }

        /* memory cell power initialization */
        SIM_array_mem_init(&arr->tag_attach_mem, info->tag_attach_mem_model,
      ...
      /* wordline power initialization */
      SIM_array_wordline_init(&arr->tag_wordline, info->tag_wordline_model,
                              info->share_rw, cols,
                              wordline_len * wordline_cmetal, info->tag_end);

      /* comparator power initialization */
      SIM_array_comp_init(&arr->comp, info->comp_model, info->tag_addr_width,
                          info->assoc,
                          SIM_array_n_pre_drain(info->comp_pre_model),
                          matchline_len, tagline_len);

      /* comparator precharging power initialization */
      SIM_array_pre_init(&arr->comp_pre, info->comp_pre_model, comp_pre_size);

      /* memory cell power initialization */
      SIM_array_mem_init(&arr->tag_mem, info->tag_mem_model, info->read_ports,
                         info->write_ports, info->share_rw, info->tag_end);
    }
